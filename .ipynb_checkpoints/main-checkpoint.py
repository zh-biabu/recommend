...
def main():
    ...
    if args.hparam_search:
        ...
        if len(param_grid) == 0:
            print("网格搜索配置为空，请检查搜索空间或 max_trials 设置。")
            return 1
        ...
        for idx, (k, v_layer, t_layer) in enumerate(param_grid):
            ...
            if args.seed is not None:
                # 使用基础 seed + 组合编号，保证可复现又有差异
                config.system.seed = args.seed + idx
        ...
        timestamp = time.strftime("%Y%m%d_%H%M", time.localtime())
        filename = f"hparam_search_grid_{timestamp}.json"
            ...
            json.dump(
                {
                    ...
                    "best_value": best_val,
                    "best_trial_number": best_summary["trial_id"] if best_summary else None,
                    "best_params": best_summary["params"] if best_summary else None,
        ...
        print("\n" + "=" * 60)
        print("Hyper-parameter search (Grid Search) finished")
        print(f"Best val metric: {best_val:.4f}")
        if best_summary:
            print("Best trial params：")
            for k_name, v_val in best_summary["params"].items():

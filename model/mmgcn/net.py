...
class GCN(nn.Module):
    def __init__(self, dim_feat, hidden_dim, emb_dim, num_users, num_items, concat, k=3, device="cpu"):
        ...
        self.ws = nn.ParameterList([nn.Parameter(nn.init.xavier_normal_(torch.randn((hidden_dim, hidden_dim), dtype=torch.float32)))])
        ...
        for _ in range(k-1):
            self.ws.append(nn.Parameter(nn.init.xavier_normal_(torch.randn((emb_dim,emb_dim), dtype=torch.float32))))
...
class Net(nn.Module):
    ...
    def forward(self, feats, node_emb, g):
        ...
        return results.mean(dim = 0), embs[0]
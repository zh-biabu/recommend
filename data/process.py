...
if __name__ == '__main__':
    # 示例：处理baby数据集
    base_dir = os.path.join(os.path.dirname(__file__), 'ori_data', 'baby')
    inter_path = os.path.join(base_dir, 'baby.inter')
    out_dir = os.path.join(os.path.dirname(__file__), "scale_data", "baby")
    df = load_inter_file(inter_path)
    print(df.columns)
    df, user2id, item2id = create_id_mapping(df, user_col="userID", item_col="itemID", out_dir=out_dir)
    train, val, test = split_data(df, time_col=None, user_col="userID")
